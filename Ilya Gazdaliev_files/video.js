var Video = {
  ...
  init: function(obj) {
    ...
    extend(cur, {
      ...
      videoPlaylists: ge('video_playlists'),
    ...
    addEvent('video_list', 'mousemove', function(event) {
      var row = event.target, limit = 10;
      while(!hasClass(row, 'video_row') && limit--) row = domPN(row);
      if (!limit) return;
  ...
  loadSilent: function(section) {
    ...
    if (list && (list.silent || list.videos)) {
      ajax.post('al_video.php', {act: 'load_videos_silent', oid: cur.oid, section: section, offset: list.silent ? 0 : list.videos.length}, {
  ...
  updateVideo: function(albumOwnerId, video, currentAlbums, deleteTotal, addedAlbums, deletedAlbums) {
    ...
    if (list) {
      ...
      toggle(ge('video_summary_tabs'), anyVideo);
  ...
  updateAlbum: function(albumId, wasDeleted, newAlbum, recalcCover, countDiff) {
    ...
    if (albumId == 'all' || albumId == 'added' || albumId == 'uploaded' || albumId == 'tagged') {
      if (countDiff && cur.videoList[albumId]) {
        ...
        var sumCount = geByClass1('video_summary_count', sumTab);
        if (sumCount) {
          val(sumCount, cur.videoList[albumId].count);
        }
  ...
  changeSummary: function() {
    ...
    if (cur.videoList[sec]) {
      ...
      var len = data.list ? (data.count ? data.count : data.list.length) : 0;
    }
  ...
  section: function(section) {
    if (section == cur.vSection) return false;
    if (!section) {
      section = 'all';
    }
  ...
  onSwitchTabs: function(section) {
    ...
    if (section == 'added' || section == 'tagged' || section == 'uploaded' || section == 'all' || section == 'search') {
      topSection = 'all';
    }
    ...
    var isInAlbum = section.substr(0, 6) == 'album_';

    toggleClass('video_list', 'video_in_playlist', isInAlbum);

    if (isInAlbum || section == 'comments' || section == 'upload') {
  ...
  updateSummaryTabs: function(forceHide) {
    if (cur.vSection == 'playlists' || cur.vSection.indexOf('album_') == 0) {
      hide('video_comments_link');
  ...
  updateTimes: function (albumsSearch) {
    if (albumsSearch && (!cur._albumsSearchRes || !cur._albumsSearchRes.length)) {
      return;
    }
    ...
    if (albumsSearch || cur.vSection == 'playlists') {
      list = albumsSearch ? cur._albumsSearchRes : cur.sections;
      timeIndex = 4;
  ...
  toggleExtendedControls: function(force) {
    toggleClass(ge('video_search'), 'video_search_extended_controls', force);
  },
  initSearchControls: function() {
    ...
    new Dropdown(ge('video_search_added'), [[0, getLang('video_search_any_time')], [86400, getLang('video_search_last_24')], [86400 * 30, getLang('video_search_last_month')], [86400 * 365, getLang('video_search_last_year')]], extend(options, {
