var VideoUpload = {
  showBox: function(params) {
    ...
    params = params || {};
    params = extend(params, {act: 'upload_box', oid: cur.oid});
    ...
    Upload.init('video_upload_box' + vars.tag, url, vars, {
      ...
      onUploadStart: function(i, res) {
        ...
        var showUploadTab = function() {
          ...
          if (geByClass1('video_upload_item_panel', tab)) {
      ...
      onUploadComplete: function(i, res) {
        ...
        var callback = function() {
          ...
          if (link) {
            ...
            link.href = rs(link.href, {'video_id': video_id});
            show(link);
  ...
  runVideoProgressUpdate: function(item, owner_id, video_id, video_hash, need_thumb, prev_progress, error_count) {
    ajax.post('al_video.php', {act: 'encode_progress', oid: owner_id, vid: video_id, hash: video_hash, need_thumb: need_thumb}, {
      onDone: function(json) {
        ...
        if (json) {
          ...
          if (need_thumb) {
            ...
            if (thumb) {
              var imgDiv = geByClass1('video_upload_thumb_placeholder', item);
              var img = geByTag1('img', imgDiv);
              if (img) {
                img.src = thumb;
              }
              need_thumb = '';
        ...
        if (needCheck) {
          setTimeout(VideoUpload.runVideoProgressUpdate.pbind(item, owner_id, video_id, video_hash, need_thumb, progress, error_count), 1000);
        }
      },
      onError: function() {
        if (!error_count) {
        ...
        setTimeout(VideoUpload.runVideoProgressUpdate.pbind(item, owner_id, video_id, video_hash, need_thumb, prev_progress, error_count + 1), error_count * 2000);
      }
    });
  },
        ...
        setTimeout(function() {
          ajax.post('/al_video.php', query, {onDone: function(data) {
            ...
            if (data.owner_id && data.video_id) {
              nav.go('/video'+data.owner_id+'_'+data.video_id, undefined, {nocur: true});
            } else {
